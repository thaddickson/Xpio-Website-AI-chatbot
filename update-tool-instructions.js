// Run this to update the Tool Instructions prompt with handoff fallback logic
// Usage: node update-tool-instructions.js <admin-password>

const adminPassword = process.argv[2];

if (!adminPassword) {
  console.error('Usage: node update-tool-instructions.js <admin-password>');
  process.exit(1);
}
  ...
  try {
    ...
    const updateResponse = await fetch(`https://xpio-website-ai-chatbot-production.up.railway.app/api/admin/prompts/${toolInstructionsPrompt.id}`, {
      ...
      body: JSON.stringify({
        ...
        description: 'Critical instructions for using the three AI tools with HANDOFF FALLBACK logic. AI can resume if human doesnt respond.',
        is_active: true
      })
    });
    ...
    console.log('✅ Successfully updated Tool Instructions with handoff fallback logic!');
    console.log('\nKey changes:');
    console.log('- AI can now resume after failed handoff');
    console.log('- AI must offer to capture contact info as fallback');
    console.log('- AI stays silent only while human is actively responding (< 2 min)');
    console.log('- No more dead conversations!');
